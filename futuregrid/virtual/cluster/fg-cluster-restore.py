...
class FgRestore:
    ...
    def __init__(self, userkey, number,
                 control_image, compute_image,
                 name, size='m1.small'):
    ...
    def euca_run_instance(self, userkey, cluster_size, image, instance_type):
        ...
        instances = [x for x in
                     ...
                     ("euca-run-instances -k %s -n %d -t %s %s"
    ...
    def euca_describe_addresses(self):
        ip_list = []
        ips = [x for x in os.popen
               ("euca-describe-addresses").read().split('\n')]
        for ip in ips:
    ...
    def create_cluster(self):
        ...
        self.euca_run_instance(self.userkey,
                               1,
                               self.control_image,
                               self.size)
        self.euca_run_instance(self.userkey,
                               int(self.compute_number),
                               self.compute_image,
                               self.size)

        ip_lists = self.euca_describe_addresses()

        # immediatly associate ip after run
        # instance may lead to error, use sleep
        time.sleep(3)
...
def usage():
    ...
    print '-s/--size    provide size of '
    'cluster(control node not included) default: s1.small'
