...
class FgRunProg:
	...
	def run_program(self):
                ...
                print '\n...Running program %s on virtual cluster %s......' %(self.file_name, self.name)

		for instance in self.cloud_instances.list()[1:]:
			self.scp(self.userkey, self.file_name, instance['ip'])
			self.ssh(self.userkey, instance['ip'], "mpicc %s -o %s" %(self.file_name, self.file_name.split('.')[0]))

		print '\n...Running program %s......' %self.file_name.split('.')[0]
		# run program on control node
		self.ssh(self.userkey, self.cloud_instances.get_by_id(1)['ip'], "salloc -N %d mpirun %s" %(int(self.number), self.file_name.split('.')[0]))
...
def main():
        ...
        try:
		opts, args = getopt.getopt(sys.argv[1:], "hu:n:f:a:", ["help", "userkey=", "number=", "file=", "name="])
